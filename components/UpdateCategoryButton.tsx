...
export default function UpdateCategoryButton({ id, name, description }: UpdateCategoryButtonProps) {
  const [state, action, pending] = useActionState(updateCategoryAction.bind(null, id), undefined);
    ...
    <Dialog>
      ...
      <DialogContent className="rounded-none sm:max-w-[425px]">
        <form action={action} className="space-y-8">
          <DialogHeader>
            <DialogTitle className="text-app-text-color">Update Category</DialogTitle>

            <DialogDescription className="text-gray-500">Change the name of the category and click save to apply the changes</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input
              ...
              name="description"
              id="description"
              defaultValue={state?.values.description ? state.values.description : description}
              className="rounded-none text-sm text-app-text-color"
          ...
          <DialogFooter>
            ...
            <Button disabled={pending} type="submit" className="rounded-none text-sm text-app-background-color hover:cursor-pointer">
              {pending ? "Updating Category..." : "Update Category"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
