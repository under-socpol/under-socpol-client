...
export default function DeleteArticleButton({ id }: { id: string }) {
  const [state, action, pending] = useActionState(deleteArticleAction.bind(null, id), undefined);
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="rounded-none sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="text-app-text-color">Delete Article</DialogTitle>

          <DialogDescription className="text-gray-500">Are you sure you want to delete this article? This action cannot be undone</DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <form action={action}>
            <Button disabled={pending} type="submit" className="rounded-none text-sm text-app-background-color hover:cursor-pointer">
              {pending ? "Deleting Article..." : "Delete Article"}
            </Button>
          </form>
        </DialogFooter>
      </DialogContent>
